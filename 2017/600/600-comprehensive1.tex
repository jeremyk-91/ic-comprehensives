...
\begin{itemize}
\item $x(y, z).P$ means that a process accepts \textit{two} inputs on channel
$x$, binds the first input to instances of $y$ and the second to instances of $z$,
and then runs $P$.
\item $\overline{x} \langle y, z \rangle. P$ means that a process sends two outputs
$y$ and $z$ on the channel $x$, and then runs $P$.
\end{itemize}

The dyadic $\pi$-calculus is actually not more expressive than the monadic
$\pi$-calculus as indicated above. This can be shown by providing an 
