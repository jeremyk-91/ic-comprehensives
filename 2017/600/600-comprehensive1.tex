...
\epigraph{Many shall be restored that now are fallen, \\
And many shall fall that are now in honor.
}{\textit{``Ars Poetica", Horace}}

\noindent In this question, we will investigate two flavours of \textit{security analysis}.

\begin{enumerate}
\item You are probably familiar with the notion of a \textit{hash function}. Let $h : \Sigma \rightarrow 2^N$ be a hash function, where $N$ is a positive integer and $\Sigma$ is a \textit{universe} of values of unbounded size. Of course, $h$ is not bijective -- we refer to the case where $i, j \in \Sigma, i \neq j$ and $h(i) = h(j)$ as a \textit{hash collision}. 
\begin{enumerate}
\item Hash functions are often used in cryptography to store message digests, to verify the content of a message. Let $h$ be a hash function, and suppose that $h$ runs in time polynomial in $\Sigma$. Then, 
\begin{itemize}
